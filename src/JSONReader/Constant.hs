...
extractConstant :: String -> Either String (JDat, String)
-- ^ 从一个字符流中提取预定义的字面量。其中的字面量由 'constAlist' 定
...
extractConstant s = maybe (Left $ "存在未知的字面量" ++ shorten s) `flip`
                    find (\ (k, _) -> k `isPrefixOf` s) constAlist     $
                    \ (k, v)
                    -> case drop (length k) s of
                         s'@(c:_) | not (isSpace c) && c `notElem` sepList
                                    -> Left ("字面量" ++ k ++ "后有非法字符" ++ [c])
                         s'         -> Right (v, s')


constAlist = [ ("true" , plainJBool True)
